...
ヘルスサーバ（`$PORT`）に、Discord ID からレーティング概要を引く読み取り専用エンドポイントを同居させてある。bot を介さず、ホスティング先（Render 等）に直接叩ける。

```
GET /api/rating/<discord_id>
```

| レスポンス | 内容 |
|---|---|
| `200` | `{"discord_id": "...", "participations": 3, "rating_highest": 1500, "rating_current": 1300}` |
| `404` | そのユーザに rating がない（未登録、または rated コンテスト未参加） |
| `400` | `discord_id` が数値でない |

- `participations` … rated コンテスト参加回数、`rating_highest` … 最高レート、`rating_current` … 現レート。
- 3値とも `rating_snapshots` 由来。`rating_current` は **減衰なし**（`/profile` の見出し値）で、`/rating` の減衰込み表示とは別物。
- 認証なし・副作用なし。`GET /`（他パス）は従来どおり状態 JSON を返す（cron-job.org のキープアライブ用）。

例: `curl https://pe-runner.onrender.com/api/rating/123456789012345678`
